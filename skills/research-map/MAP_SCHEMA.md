...
| `next` | | 이 노드에서 더 파볼 것. 문자열, 또는 아래 객체 |
...
문자열이면 그대로 쓰이고, 객체면 **열린 질문** 탭이 우선순위를 매길 수 있다.

```json
"next": [
  "그냥 적어도 된다",
  {"text": "D2 판정", "due": "2026-11-30", "effort": "long",
   "blockedBy": ["th-d1-verdict", "지도교수 판단"], "why": "사전 고정 게이트 ②"}
]
```
...
|---|---|
| `text` | 필수. 할 일 |
| `due` | `YYYY-MM-DD`. 사전 고정 게이트처럼 날짜가 박힌 것에만 |
| `effort` | `quick`(한 시간 안) · `day`(하루) · `long`(그 이상) |
| `blockedBy` | 노드 id 또는 자유 문자열. **하나라도 있으면 "기다리는 것"으로 내려간다** |
| `why` | 한 줄 이유 |

기한·비용을 안 적으면 "아직 기한·비용을 안 적은 것"으로 접혀 들어간다. 모르면 비워 둬라 —
**추측해서 채우면 우선순위가 거짓말을 한다.** 실제로 근거가 있는 것만 적는다.
