...
**추출이 세 갈래로 나눠 알려준다. 그대로 따른다.**

```
SKILL.md 2단계 — 이번에 손댈 세션 4개
  새 세션 2개 — 카드를 새로 쓴다: 66c9ef43, 9a34d5ca
  이어진 세션 1개 — 기존 카드 + 아래 줄부터만 읽고 **갱신**한다:
    6ea243f8  ...\digests\...\6ea243f8….md  offset=8432줄 (12,400자 추가 / 전체 629,492자)
  digest 만 다시 만든 세션 1개 — **이미 카드가 있으니 그냥 두라**.
```

**(가) 새 세션** — 서브에이전트(general-purpose, 백그라운드)에게 digest 전체를 읽히고
`CARD_SCHEMA.md` 대로 `maps/<지도>/cards/<sid8>.json` 을 쓰게 한다. 프롬프트에 넣을 것:
...
**(나) 이어진 세션** — 대화가 계속됐을 뿐이므로 **기존 카드를 갱신**한다. 서브에이전트에게:
- 먼저 `cards/<sid8>.json` 을 읽어 이미 무엇이 적혀 있는지 파악
- digest 는 **`offset=` 이 가리키는 줄부터만** 읽는다 (`Read` 의 `offset` 인자)
- 새 내용이 기존 thread 의 진전이면 그 thread 의 `steps`·`outcome`·`keyNumbers` 에 **덧붙이고**,
  판정이 뒤집혔으면 `status` 를 바꾼다. 새 갈래면 thread 를 추가한다
- **기존 내용을 지우지 마라.** 앞부분을 안 읽었으므로 없애야 할 근거가 없다
- `period` 끝 날짜와 `sessionIds` 를 갱신

**(다) `digest 만 다시 만든 세션`** — 이미 카드가 있다. **그냥 둔다.** 기록이 통째로
바뀌었거나 `--all` 을 썼을 때 나오는 줄이지, 내용이 달라졌다는 뜻이 아니다.

앞 절반을 다시 읽지 않는 것이 이 단계의 전부다. 63만 자 세션에 1만 자가 붙었으면
1만 자만 읽는다. 갈래를 무시하고 전부 새로 쓰면 비용이 수십 배가 된다.
...
- extract 가 **`사본 후보`** 를 찍으면(첫 프롬프트가 다른 세션과 같고 시각이 하나뿐 = 포크·재개
  사본) 그 세션의 내용은 이미 원본 세션으로 지도에 들어가 있다. **원본을 다룬 노드에 출처만
  붙이고**, 카드가 더 세밀한 부분만 세부 노드로 쪼갠다. 노드를 새로 만들지 않는다.
- render 가 **`중복 의심`** 쌍을 찍으면 제목이 거의 같은 노드가 둘 있다는 뜻이다. 같은 일이면
  한쪽에 합치고(evidence·sources 를 옮기고 삭제), 다른 일이면 `links` 로 서로 이어 경고를 끈다.
