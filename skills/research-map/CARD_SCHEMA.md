...
대화가 이어졌을 뿐이면 카드를 새로 쓰지 말고 고친다. 추출기가 digest 의 몇 줄부터가
새 내용인지 알려주므로 **그 뒤만** 읽는다.

- 기존 thread 의 진전 → 그 thread 의 `steps` 에 줄을 덧붙이고, `outcome`·`keyNumbers` 를 갱신
- 판정이 뒤집혔으면 `status` 를 바꾸고 `steps` 에 날짜와 이유를 남긴다
- 새 갈래가 시작됐으면 thread 를 추가한다
- `period` 끝 날짜와 `sessionIds` 갱신
- **기존 내용을 지우지 않는다** — 앞부분을 다시 읽지 않았으므로 지울 근거가 없다
